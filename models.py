        ...
        for row_index, row in tqdm.tqdm(
            ...
            if response_str is None:
                ...
                if len(response_str) >0:
                    ...
                    logger.info(f'ans_list is:{ans_list}')
                    logger.info(f'row_answer is:{row['answer']}')
                ...
                model_ans.append(ans_list[0])
                score.append(correct)
        correct_ratio = 100 * correct_num / len(answers)
        logger.info(f'{correct_ratio}')
        test_df["model_output"] = result
        test_df["model_answers"] = model_ans
        test_df["correctness"] = score
        test_df.to_csv(
            os.path.join(save_result_dir, f"{subject_name}_val.csv"),
            encoding="utf-8",
            index=False)
        return correct_ratio, test_df
#%%
